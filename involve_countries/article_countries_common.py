...
def one_xml(file_loc):
    ...
    speech = collection.getElementsByTagName('speech')
    thisfile_country = {}
    ...
    for node in speech:
       ...
       if temp!=[]:
            ...
            p_id.append(node.getAttribute('id'))

    return thisfile_country


xml_files = []
for root, dirs, files in os.walk("C:\\Users\\hjn\\Desktop\\议会数据\\议会数据\\英国议会数据xml格式\\commons"):
...
with open("C:\\Users\\hjn\\Desktop\\大创项目准备\\提取国家\\段落涉及国家（commons).csv", "w", encoding='utf-8', newline='') as csvfile:
...
# with open("C:\\Users\\hjn\\Desktop\\大创项目准备\\提取国家\\全文涉及国家（commons）.csv", "w", encoding='utf-8', newline='') as csvfile:
#     csvwriter = csv.writer(csvfile)
